...
pub fn prepare_volume_root(
    ...
    oci::write_rootfs_file(
        ...
        generate_init(ep, net, persistent, mounts, disks).as_bytes(),
        0o755,
    )?;
...
/// A block device (`--attach-disk PATH:/guest/path`) the generated init should
/// mount. Unlike a virtio-fs share this is a real filesystem the guest owns,
/// which is what a container runtime's storage wants: overlay on ext4, sized
/// and grown independently of the host's rootfs.
#[derive(Debug, Clone)]
pub struct DiskMount {
    /// Which attached disk this is, in `--attach-disk` order. The guest names
    /// them `/dev/vda`, `/dev/vdb`, … but resolving the *i-th* device inside
    /// the guest survives libkrun naming them differently.
    pub index: usize,
    pub guest: String,
    pub ro: bool,
}
...
fn generate_init(
    ...
    // Attached block devices (`--attach-disk PATH:/guest/path`). A blank image
    // is formatted on first boot, and `resize2fs` every boot picks up an image
    // the host has since grown — the guest half of `bsdkrun grow`.
    for d in disks {
        let g = sh_quote(&d.guest);
        let nth = d.index + 1;
        s.push_str(&format!(
            "dev=$(ls /dev/vd? 2>/dev/null | sort | sed -n '{nth}p')\n"
        ));
        s.push_str("if [ -n \"$dev\" ]; then\n");
        s.push_str(&format!("  mkdir -p {g} 2>/dev/null\n"));
        if !d.ro {
            // Keep mkfs's own stderr: when formatting fails, the reason is the
            // only thing that turns "mount failed: Invalid argument" back into
            // an actionable message.
            // Test blkid's *output*, not its exit status: busybox's blkid
        ...
        // so an unspecified type on a freshly formatted disk fails with a bare
        // "Invalid argument".
        s.push_str(&format!(
            "  mount -t ext4{opt} \"$dev\" {g} || mount{opt} \"$dev\" {g} || \
             echo '[bsdkrun] mount {g} failed'\n"
        ));
        if !d.ro {
            s.push_str("  resize2fs \"$dev\" >/dev/null 2>&1 || true\n");
        }
        s.push_str("fi\n");
    }
    if net {
