...
try {
  ...
  const listing = await sbx.sh`ls ${dir} 2>&1 || echo "no such dir"`.text();
  console.log("safe listing:", listing);

  // `.text()`, `.json()`, `.lines()` are convenience accessors.
  await sbx.exec(["apk", "add", "--no-cache", "jq"]).catch(() => {});
  const os = await sbx.sh`cat /etc/os-release | head -2`.lines();
  console.log("os lines:", os);

  // `.nothrow()` keeps a non-zero exit from throwing.
  const missing = await sbx.sh`cat /nope`.nothrow();
  console.log("missing exit:", missing.exitCode);

  // `.env()` sets variables just for this command.
  const greet = await sbx.sh`echo "$GREETING world"`.env({ GREETING: "hello" }).text();
  console.log(greet);

  // `raw()` splices a value in WITHOUT quoting (trusted content only).
  const flags = raw("-la /var");
  const ll = await sbx.sh`ls ${flags} | head -3`.text();
  console.log(ll);
} finally {
  await sbx.stop();
}
