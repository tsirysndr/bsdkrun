...
pub fn build_initramfs(rootfs: &Path, ep: &Entrypoint, net: bool) -> Result<PathBuf> {
    ...
    oci::write_rootfs_file(&initstage, "init", generate_init(ep, net).as_bytes(), 0o755)?;
...
fn generate_init(ep: &Entrypoint, net: bool) -> String {
    ...
    s.push_str("sync\n");
    // Prefer a real poweroff if the image has one; else magic-sysrq; else fall
    // back to an interactive shell rather than panic-looping on init exit.
    s.push_str(
